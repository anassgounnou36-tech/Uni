...
import type { NormalizedOrder, OrderReasonCode, OrderStore, StoredOrderRecord, UpsertResult } from '../types.js';
...
export class PostgresOrderStore implements OrderStore {
  ...
  upsertDiscovered(rawPayload: unknown, normalizedOrder: NormalizedOrder | undefined, nowMs?: number): UpsertResult {
    const result = this.memory.upsertDiscovered(rawPayload, normalizedOrder, nowMs);
    if (result.created) {
